...
// action creators are just functions that are called to dispatch specific actions
// these actions are sent to the specific reducer to produce a result (update state)
  ...
  addCard(card) {
    return dispatch => {
      ...
      KanbanAPI.addCard(card).then(
        receivedNewCard =>
          dispatch({ type: RECEIVE_CREATE_CARD, success: true, card: receivedNewCard }),
