  ...
  deleteTask(cardId, task, taskIndex) {
    return dispatch => {
      ...
      KanbanAPI.deleteTask(cardId, task).then(
        ...
        error =>
          dispatch({ type: RECEIVE_DELETE_TASK, success: false, cardId, task, taskIndex, error }),
      );
    };
  },
  ...
  toggleTask(cardId, task, taskIndex) {
    return dispatch => {
      ...
      KanbanAPI.toggleTask(cardId, task).then(
        ...
        error => dispatch({ type: RECEIVE_TOGGLE_TASK, success: false, carId, taskIndex, error }),
      );
    };
  },
};
